...
class OLED:
    ...
    def init(self):
        ...
        print wiringpi.wiringPiSPISetup(0, 1000000)
        wiringpi.pinMode(self.PIN_DC,  wiringpi.OUTPUT)
        wiringpi.pinMode(self.PIN_CS,  wiringpi.OUTPUT)
        wiringpi.pinMode(self.PIN_RST, wiringpi.OUTPUT)
        wiringpi.digitalWrite(self.PIN_DC, 0)
        wiringpi.digitalWrite(self.PIN_RST, 0)
        wiringpi.digitalWrite(self.PIN_CS, 1) # deselect
        
        wiringpi.digitalWrite(self.PIN_RST, 1)
        time.sleep(0.001)
        wiringpi.digitalWrite(self.PIN_RST, 0)
        time.sleep(0.010)
        wiringpi.digitalWrite(self.PIN_RST, 1)
    ...
    def drawString(self, x, y, text, font):
        for i in xrange(len(text)):
            self.drawChar(x + (l * (font.width + 1)), y, text[l], font)
...
d.init()
d.clear()
d.drawString(0, 11, "1BRMLAB7nryYgFGrG8x9S", Font5x8)
d.drawString(0, 21, "12381.87431928 B", Font7x8)
d.drawString(0, 56, "Confirm        Cancel", Font5x8)
d.invert(0, 20, 127, 28)
